...
function sensing_participate_table ( data) {
   head= participate_head
   div_name="sensing_participate_table"
    $('#'+div_name).empty();
	// if(data.length>6){
	// 	$('#'+div_name).css("overflow-y", "auto");
	// }
	...
	html += '<table id="'+div_name+'_table" style="word-wrap: break-word;word-break: break-all;table-layout:fixed;margin-bottom:0px;" class="table  table-striped">';// class="table table-bordered table-striped table-condensed datatable"
	html += '<thead><tr>';
	for(var i=0; i<head.length; i++){
		html += '<th style="text-align:center">'+head[i]+'</th>';
	}
 ...
 //participate_head=['用户ID','昵称','领域','话题','重要度','影响力','活跃度']
	...
	$('#'+div_name).append(html);
     $('#sensing_participate_table_table').dataTable({
        "sScrollX": "100%",
        "sScrollXInner": "500%",
        "sScrollY":"300",
...
function sen_out_table ( data) {
    head=out_head;
    div_name="out_participate_table"
    $('#'+div_name).empty();
    // if(data.length>6){
    //     $('#'+div_name).css("overflow-y", "auto");
    // }
	...
	html += '<table id="'+div_name+'_table" style="table-layout:fixed;margin-bottom:0px;" class="table  table-striped">';// class="table table-bordered table-striped table-condensed datatable"   word-wrap: break-word;word-break: break-all;
    html += '<thead><tr>';
    var width_set = ["140","110","100","55","55"]
    for(var i=0; i<head.length; i++){
            html += '<th style="width:'+width_set[i]+'px;text-align:center">'+head[i]+'</th>';
    }
    html += '<th style="text-align:center;width:25px"><input name="sen_out_choose_all" id="sen_out_choose_all" type="checkbox" value="" onclick="sen_out_choose_all()" /></th>'
    html += '</tr></thead>';
    html += '<tbody>';
 //participate_head=['用户ID','昵称','领域','话题','热度','重要度','影响力','活跃度']
    ...
    // });
    $('#'+div_name).append(html);
     $('#out_participate_table_table').dataTable({
        "sScrollX": "100%",
        "sScrollXInner": "520%",
        "sScrollY":"300",
        "sScrollCollapse":false,
...
function page_group_weibo(start_row, end_row, data, div_name, sub_div_name){
    ...
    for (var i = start_row; i < end_row; i += 1){
       ...
       // }
        ...
        html += '</div>';
        html += '<div class="weibo_pz" style="margin:0px;padding-left:10px;">';
        html += '<div class="m">';
        html += '<u>' + date + '</u>&nbsp;-&nbsp;';
        ...
        html += '<div class="m">';
        html += '<u>' + date + '</u>&nbsp;-&nbsp;';
        html += '<a target="_blank" href="' + user_link + '">用户</a>&nbsp;&nbsp;-<a href="javascript:void(0)" id="show_sim" value="'+mid+'">相似微博</a>';
        html+= '</div></div>'
        html += '</div>';
        html += '</li>';
...
function draw_userinfo(data){
    ...
    sensing_participate_table(user_detail);
    sen_out_table(out_user);
...
sensing_url += '/social_sensing/get_warning_detail/?task_name='+task_name+'&user='+sensing_user+'&ts='+ts;
var trendline_url = '/social_sensing/get_trendline/?task_name='+task_name+'&user='+sensing_user+'&ts='+ts;
var userinfo_url = '/social_sensing/get_user_info/?task_name='+task_name+'&user='+sensing_user+'&ts='+ts;
//sensing_url += '/social_sensing/get_warning_detail/?task_name='+task_name+'&user='+sensing_user+'&ts='+ts;
    var participate_head=['用户ID','昵称','身份','领域','影响力'];
    var out_head=['用户ID','昵称','注册地','粉丝数','影响力'];
//call_sync_ajax_request(sensing_url, social_sensing_all);
call_sync_ajax_request(trendline_url, draw_trendline);
call_sync_ajax_request(userinfo_url, draw_userinfo);
//call_sync_ajax_request(sensing_url, social_sensing_all);
//call_sync_ajax_request(sensing_url, social_sensing_all);
    $('#in_more').click(function(e){
        url = "/social_sensing/get_more_portrait/?task_name=" + task_name+'&user='+user+'&ts='+ts;
      call_sync_ajax_request(url, sensing_participate_table);
    });
    $('#out_more').click(function(e){
        url = "/social_sensing/get_more_out/?task_name=" + task_name+'&user='+user+'&ts='+ts;
      call_sync_ajax_request(url, sen_out_table);
    });
