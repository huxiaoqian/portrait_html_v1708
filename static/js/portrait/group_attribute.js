...
function getAttributeName(){
    var attribute_name_url = '/tag/show_attribute_name/?user='+$('#useremail').text();
    $.ajax({
        type:'GET',
    ...
    function draw_name_option(data){
        ...
        $('[name=tag_type]').change(function(){
            if ($(this).val() == ''){
                ...
                var attribute_value_url = '/tag/show_attribute_value/?user='+$('#useremail').text()+'&attribute_name=';
...
function attr_bind_click(){
    $('#attribute_pattern #show_advanced').click(function(){
        if($('#attribute_pattern #advanced_condition').is(':hidden')){
            ...
            $('#attribute_pattern #advanced_condition').removeClass('hidden');
        }
        else{
            $(this).html('高级');
            $('#attribute_pattern #advanced_condition').css('display', 'none');
            $('#attribute_pattern #advanced_condition').addClass('hidden');
        }
    });
...
function attribute_pattern_check(){             // check validation 
  ...
  return true;
    ...
    if(flag){
     ...
     if($('#attribute_pattern #advanced_condition').hasClass('hidden')){ 
      ...
      if ($('[name="tag_type"]').val() != ''){
          tag_url += 'tag=' + $('[name="tag_type"]').val() + '-' + $('[name="tag_name"]').val();
      ...
      */
      ...
      url_attribute += url_all.join('&') + '&submit_user=' + $('#useremail').text()+'&task_max_count='+group_max_count;
