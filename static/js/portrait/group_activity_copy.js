...
function call_sync_ajax_request_location(url, method, callback){
    $.ajax({
          url: url,
                type: method,
                      dataType: 'JSONP',
            ...
            series:{            
                ...
                events:{
                    click:function(event){
                        ...
                        $('#line_content').empty();
                        var loadingHTML = '数据正在加载中，请稍后...';
                        $('#line_content').append(loadingHTML);
...
function show_online_time(data){
    ...
    for (var i = 0; i < online_time_data.length; i++) {
       html += '<td style="text-align:center">' + online_time_data[i] + '</td>';
    };
    html += '</tr></table>'; 
...
function moving_geo(data,data2){
    ...
    if (dealt_data[0].length == 0){
        ...
        if(dealt_data[0].length < 5){
            ...
            html += '</tr>';
            if(dealt_data[0].length>=5){
                dealt_data[0].length = 5;
            }
            for (var i = 0; i < dealt_data[0].length; i++) {
                html += '<tr>';
                html += '<td style="text-align:center;vertical-align: middle;font-size:10px;min-width:40px;" >' +fromCity[i] + '</td>';
                html += '<td style="text-align:center;"><img src="/../../static/img/arrow_geo.png" style="width:25px;"></td>';
                html += '<td style="text-align:center;vertical-align: middle;font-size:10px;min-width:40px;" >' + endCity[i] + '</td>';
...
function Draw_more_moving_geo(from_city, end_city, dealt_data,fromCity,endCity){
    ...
    //console.log(dealt_data[0].length);
    for (var i = 0; i < dealt_data[0].length;i++) {
        html += '<tr>';
...
var dict_lng = new Object();
var city = [];
var ll = [];
var i=0;
function getLng(city, i){
    //console.log(city);
    //console.log(i+":"+city[i]);
    var search_lng = 'http://api.map.baidu.com/geocoder/v2/?address='+city[i]+'&output=json&ak=2gsxNh6TEkokMma1UdaR35CDG7e843ze&callback=showLocation';
        //call_sync_ajax_request_location(search_lng,ajax_method,lng);
        $.ajax({
              url: search_lng,
              type: 'GET',
              dataType: 'JSONP',
              ...
              success: function(data){
                  dict_lng[city[i]] = [data['result']['location']['lng'], data['result']['location']['lat']];
                 //console.log(dict_lng);
                 //setTimeout(4000);
...
for(var i=0;i<from_city.length;i++){
    var search_lng = 'http://api.map.baidu.com/geocoder/v2/?address='+from_city[i]+'&output=json&ak=Sz7fnFrQjivoX0u9pnQjXkM05B323EzO&callback=showLocation';
   var city = from_city[i];
   setTimeout(400);
   call_sync_ajax_request_location(search_lng, ajax_method,function(data){getLng(data,city)});
}
for(var i=0;i<end_city.length;i++){
    var search_lng = 'http://api.map.baidu.com/geocoder/v2/?address='+end_city[i]+'&output=json&ak=Sz7fnFrQjivoX0u9pnQjXkM05B323EzO&callback=showLocation';
       //console.log(search_lng);
       call_sync_ajax_request_location(search_lng, ajax_method,function(data){getLng(data,end_city[i])});
}
*/
for(var i=0;i<from_city.length;i++){
    city.push(from_city[i]);
}
for(var i=0;i<end_city.length;i++){
    city.push(end_city[i]);
}
getLng(city, 0);
setTimeout('cityLWL()',4000);
//cityLW();
//console.log(dict_lng);
   ...
   for(var i=0;i<from_city.length;i++){
    ...
    function drawroute(){
        var route = new Array();
        for(var i=0;i<from_city.length;i++){
            if(dict_lng[from_city[i]]!='undefined'&& dict_lng[end_city[i]]!='undefined'){
...
function Draw_top_place(start,end){
    ...
    $('#track_weibo_in').empty();
    var html = '';
    var a ='局域网';
    html += '<select id="select_track_weibo_in" style="max-width:150px;">';
    for (var i = 0; i < start.length; i++) {
    if(start[i][0].indexOf(a)>=0){
    continue;
    }
        html += '<option value="' + start[i][0] + '">' + start[i][0] + '</option>';
    }
    html += '</select>';
    $('#track_weibo_in').append(html);

    $('#track_weibo_out').empty();
    var html = '';
    html += '<select id="select_track_weibo_out" style="max-width:150px;">';
    for (var i = 0; i < end.length; i++) {
    if(end[i][0].indexOf(a)>=0 ){
    continue;
    }
        html += '<option value="' + end[i][0] + '">' + end[i][0] + '</option>';
    }
    html += '</select>';
    $('#track_weibo_out').append(html);
    var html = '';
	$('#top_place').empty();
    var html = '';
    if(start.length==0){
		...
		if(start.length <=5){
		    html += '<div > <span style="font-weight:bold;width:100%;line-height:30px;">主要出发地：</span></br>';
		    for(var i=0;i<start.length;i++){
			    html = html + '<span style="width:45%;line-height:30pxfloat:left">' +start[i][0] +'('+start[i][1]+'人次)</span>&nbsp&nbsp';
		    }
		    html += '</div>'
	    }else{
		    html += '<div > <span style="font-weight:bold;width:100%;line-height:30px;">主要出发地：</span></br>';
		    for(var i=0;i<5;i++){
			    html = html + '<span style="width:45%;line-height:30px;float:left">' +start[i][0] +'('+start[i][1]+'人次)</span>&nbsp&nbsp';
		    }
		    html += '<a style="cursor:pointer;width:45%;line-height:30px;float:left" id="more_start" data-toggle="modal" data-target="#moreStart">更多</a>';
			Drawmoreplace(start,'start_WordList');
	    }
	}
		
	if(end.length==0){
		html += '<div style="float:left;font-weight:bold;width:100%;line-height:30px;">主要目的地：暂无数据</div>';
	}else {
		
		if(end.length <=5){
		    html += '<div > <span style="float:left;font-weight:bold;width:100%;line-height:30px;">主要目的地：</span></br>';
		    for(var i=0;i<end.length;i++){
			    html = html + '<span  style="width:45%;line-height:30px;float:left">' +end[i][0] +'('+end[i][1]+'人次)</span>&nbsp&nbsp';
		    }
		    html += '</div>'
	    }else{
		    html += '<div > <span style="float:left;font-weight:bold;width:100%;line-height:30px;">主要目的地：</span></br>';
		    ...
		    html += '<a style="cursor:pointer;width:45%;line-height:30px;float:left" id="more_end" data-toggle="modal" data-target="#moreEnd">更多</a></div>';
...
function show_activity_track(data){
    ...
    var choose_add = $('#select_track_addition option:selected').val();
    var html = '';
    html += '<select id="select_track_weibo_user" style="max-width:150px;">';
        html += '<option value="all_user">全部用户</option>';
    for (var i = 0; i < data.length; i++) {
        if(data[i][1]=='unknown'){
            data[i][1] = data[i][0];
        }
...
// var activity_data = []
$(function(){
    $('#select_track_addition').change(function(){
    var choose_add = $('#select_track_addition option:selected').val();
    var html = '';
    if(choose_add=='select_user'){
      $('#track_weibo_user').css('display','block').siblings().css('display','none');
    }else if(choose_add=='select_out'){
      $('#track_weibo_out').css('display','block').siblings().css('display','none');
    }else{
      $('#track_weibo_in').css('display','block').siblings().css('display','none');
    }
});
});
