...
$('#event_pattern #show_advanced').click(function(){
    if($('#event_pattern #advanced_condition').is(':hidden')){
        ...
        $('#event_pattern #advanced_condition').removeClass('hidden');
        
    }
    else{
        $(this).html('高级');
        $('#event_pattern #advanced_condition').css('display', 'none');
        $('#event_pattern #advanced_condition').addClass('hidden');
...
function event_pattern_check(){             // check validation 
  ...
  if (impor_from_num > impor_to_num){
    alert('身份敏感度输入栏左侧输入值应小于右侧，请重新输入！')
    return false;
  }
...
function submit_event(){
	if(event_pattern_check()){
        ...
        if ($('#event_pattern #advanced_condition').hasClass('hidden')){
            //console.log('hidden');
            event_all_url.push('extend_mark=0');
        }else{
            event_all_url.push('extend_mark=1');
      	    var domain_url = '';
		...
		event_pattern_url += event_all_url.join('&') + '&submit_user=' + $('#useremail').text() + '&task_max_count='+group_max_count;
