...
d_Tag_show.prototype = {   //获取数据，重新画表
...
d_Draw_tag:function(data){
	//console.log('d_Draw_tag',data);
	var item;
	var name;
	var value;
	var attrTag = [];
    ...
    if(item.length==0){
      html += '<div style="line-height: 40px;" class="">暂无数据</div>';
    }else{
    html += '<div style="width:100%;height:20px">';
	for(i=0;i<item.length;i++){
		html += '<div class="tagClo fleft" style="padding:0px;margin-bottom:0px" ><span class="ptagName" style="color:blue;">'+attributeNames[i]+'</span>:<span class="tagbg"><span>'+attributeValues[i]+'<span><a id="d_delIcon" ></a></span></div>';	
	}
    html += '</div>';}
	$('#d_tag').append(html);    
   }
...
var d_Tag_show = new d_Tag_show();
d_Tag_show.call_sync_ajax_request(url, d_Tag_show.ajax_method, d_Tag_show.d_Draw_tag);
//选择类别
function Show_name(){
  this.ajax_method = 'GET';
}
...
$('#d_select_attribute_name').change(function(){
    var select_attribute_name = $("#d_select_attribute_name").val();
       //console.log('d_select_attribute_name !=)',select_attribute_name ,select_attribute_name !='');
    if(select_attribute_name !=''){
...
function deleteTag(that){
	$('a[id^=d_delIcon]').live('click', function(){
		...
		del_url = del_url +'uid=' + uid + '&attribute_name='+delname+'&user='+$('#d_useremail').text();
        //console.log(del_url);
		that.call_sync_ajax_request(del_url, Tag_del.ajax_method, Tag_del.d_Draw_tag_del);
	});
}
var Tag_del = new Tag_del();
deleteTag(Tag_del);
//console.log('dddd',$('#d_useremail').text());
