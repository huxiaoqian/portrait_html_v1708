function pattern_commit(){
    if(pattern_check()){
      ...
      geo_content = geo_content.split(' ').join(',');
      if(geo_content != ''){
      ...
      ip_content = ip_content.split(' ').join(',');
      if(ip_content != ''){
      ...
      if($('#pattern #advanced_condition').hasClass('hidden')==false){
      var num_range_url = '';
      ...
      url_pattern += url_all.join('&') + '&submit_user=' + $('#useremail').text()+'&task_max_count='+group_max_count;
...
function pattern_bind_click(){
    $('#pattern #show_advanced').click(function(){
        if($('#pattern #advanced_condition').is(':hidden')){
            ...
            $('#pattern #advanced_condition').removeClass('hidden');
        }
        else{
            $(this).html('高级');
            $('#pattern #advanced_condition').css('display', 'none');
            $('#pattern #advanced_condition').addClass('hidden');
        }
    });
