...
Draw_overview: function(data){
    ...
    if(data.state == ''){
        state = '<span id="g_states">无此数据</span>';
    }else if(data.state.length>11){
        state = '<span id="g_states" title="'+data.state+'">'+ data.state.substr(0,10)+'...</span>';
    }else{
        state='<span id="g_states">'+data.state+'</span>';
    };
...
if(name.length>12){
html += '<p style="font-size:13px;padding: 5px 5px 5px 5px;">群体名称：<a style="cursor:pointer;" onclick="group_table();" title="'+name+'" >' + name.substr(0,10) +'...</a></p>';
}else{
html += '<p style="font-size:13px;padding: 5px 5px 5px 5px;">群体名称：<a style="cursor:pointer;" onclick="group_table();" title="'+name+'" >' + name +'</a></p>';
};
    html += '<p style="font-size:13px;padding: 5px 5px 5px 5px;">创建时间：' + submit_date +'</p><p style="font-size:13px;padding: 5px 5px 5px 5px;">创建人员：' + submit_user +'</p><p style="font-size:13px;padding: 5px 5px 5px 5px;" id="group_states">群体备注：' + state +'</p>';
    html += '</a></li></ul></div>';
    ...
    html += '<td style="padding:0px;font-size:14px;text-align:center;vertical-align:middle"><b>&nbsp;&nbsp;&nbsp;&nbsp;活跃度<i id="" class="glyphicon glyphicon-question-sign" data-toggle="tooltip" data-placement="right" title="衡量群体内部成员线上线下的活跃程度，通过发布微博总数、活跃地区数、发布微博的时间走势计算得到"></i>&nbsp;&nbsp;</b></td>';

    html += '<td style="padding:0px;font-size:14px;text-align:center;vertical-align:middle"><b>&nbsp;&nbsp;&nbsp;&nbsp;身份敏感度<i id="" class="glyphicon glyphicon-question-sign" data-toggle="tooltip" data-placement="right" title="衡量群体内部成员对社会网络安全业务的重要程度，通过群体成员的所属领域和偏好话题计算得到"></i>&nbsp;&nbsp;</b></td>';
    ...
    //html += '<tr><td><span style="color:red;margin-left:20px;font-size:13px;">身份敏感度<i id="" class="glyphicon glyphicon-question-sign" data-toggle="tooltip" data-placement="right" title="衡量群体内部成员对社会网络安全业务的重要程度，通过群体成员的所属领域和偏好话题计算得到"></i></span><span>&nbsp;'+density_star+'</span></td>';
...
function add_group_tag(){
    ...
    //console.log(select_uids_string);
      //add_tag_attribute_name = $("#attribute_name").val();
    //add_tag_attribute_value = $("#attribute_value").val();
...
$(document).ready(function(){
    ...
    var o_user = $('#g_useremail').text();
    var group_overview_url = '/group/show_group_result/?module=overview&task_name=' + name+'&submit_user='+o_user;
     Search_weibo.call_sync_ajax_request(group_overview_url, Search_weibo.ajax_method, Search_weibo.Draw_overview);
    var tag_url =  'http://' + downloadurl + "/tag/show_attribute_name/?user="+o_user;
    ...
    try{
    attribute_value_url = '/tag/show_attribute_value/?attribute_name=' + select_attribute_name+'&user='+$('#g_useremail').text();
    //console.log(attribute_value_url);
...
g_bind_remark_edit(Search_weibo);
    ...
    $('#select_attribute_name').change(function(){
            ...
            attribute_value_url += '&user='+$('#g_useremail').text();
            //console.log(attribute_value_url);
            Search_weibo.call_sync_ajax_request(attribute_value_url, Search_weibo.ajax_method, Search_weibo.Draw_attribute_value);
        }); 
    }catch(err){
...
function group_table(){
    var model_url =   "/group/show_group_list/?task_name=" + name+'&submit_user='+$('#g_useremail').text();
    ...
    function Draw_model(data){
        ...
        html += '<th class="center" style="text-align:center">注册地</th><th class="center" style="text-align:center">身份敏感度</th><th class="center" style="text-align:center;width:72px">影响力</th>';
