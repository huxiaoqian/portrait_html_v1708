function seed_user_callback(data){
    ...
    if(data == 'no enough user to analysis'){
        ...
        if (data[0] == 'invalid seed user'){
            alert('用户人数太少，无法提交任务！');
            return;
        }else if(data[0]=='invalid user info'){
            var vaild_user = [];
            var vaild_string = '无效用户信息：';
            for(var i=0;i<data[1].length;i++){
                vaild_string += ' '+data[1][i];
            }
            alert(vaild_string);
            return
        }
    ...
    $('#seed_user #show_advanced').click(function(){
        if($('#seed_user #advanced_condition').is(':hidden')){
            ...
            $('#seed_user #advanced_condition').css('display', 'block');
             $('#seed_user #advanced_condition').removeClass('hidden');
        }
        else{
            $(this).html('高级');
            $('#seed_user #advanced_condition').css('display', 'none');
            $('#seed_user #advanced_condition').addClass('hidden');
        }
    });
...
function seed_single_user_data(){
    var url = '';
    //console.log($('#content_choose').val());
...
function seed_multi_user_data(){
    ...
    if ($('#advanced_condition').hasClass('hidden')){
        upload_job['extend'] = '0';
    }
    else{
        upload_job['extend'] = '1';
    ...
    upload_job['task_max_count'] = group_max_count;
