...
Draw_dis_Table:function(data){
	...
	html += '<table id="dis_table_body" class="table table-bordered table-striped table-condensed datatable" style="table-layout:fixed">';
    html +='<thead><tr style="text-align:center;">';
    html += '<th style="width:150px;">群组名称</th>';
    html += '<th style="width:115px;">提交人</th>';
    html += '<th style="width:90px;">时间</th>';
    html += '<th style="width:115px;">发现方式</th>';
    html += '<th>备注</th>';
    html += '<th>进度</th>';
    html += '<th style="width:165px;">操作</th>';
    html += '</tr></thead>';
	html += '<tbody>';
	//j = 40;
	var dis_type='';
	for (i=0;i<data.length;i++){
		...
		html += '<tr><td title='+data[i][0]+' style="word-break:break-all;">'+data[i][0]+'</td><td style="word-break:break-all;">'+data[i][1]+'</td><td>'+data[i][2]+'</td><td>'+dis_type+'</td><td>'+data[i][4]+'</td><td><progress style="width:78px;" value="'+data[i][5]+'" max="100"></progress>&nbsp;&nbsp;'+data[i][5]+'%</td><td><a href="javascript:void(0)" id="group_commit_analyze">提交分析</a>&nbsp;&nbsp;&nbsp;';
        if (user_roles != 'False'){
        html += '<a href="javascript:void(0)" id="group_commit_control" >提交监控</a>&nbsp;&nbsp;';}
        html += '<a href="javascript:void(0)" id="task_del">删除</a></td></tr>';
		//j += 10;
	}
	html += '</tbody>';
	html += '</table>';
...
function submit_analyze(that){
	...
	$('a[id^="group_commit_analyze"]').click(function(e){
		...
		if(percent.replace(/[^0-9]/ig,"") != 100){
            ...
            url += '&submit_user=' + $('#useremail').text();
            url += '&task_max_count='+ group_analysis_count;
...
function draw_control_table(data){
    ...
    html += '<th style="text-align:center;width:60px;">身份敏感度</th>';
...
function draw_table(data,div){
    ...
    $('#group_num').css('display','none');
     ...
     $('#group_num').css('display','block');
	$(div).empty();
	//var datas = data['topic'];
    html = '';
    html += '<table class="table table-striped table-bordered bootstrap-datatable datatable responsive" style="overflow-y:auto;max-height:300px;">';
    html += '<tr><th style="text-align:center">用户ID</th>';
    ...
    html += '<th><input name="analyze_choose_all" id="analyze_choose_all" type="checkbox" value="" onclick="analyze_choose_all()" /></th></tr>';
    for (var i=0;i<data.length;i++) {
        var uname = data[i][1];
        if (uname == 'unknown' || uname == null || uname == ''){
            uname = data[i][0];
        }
    ...
    html += '</table>'; 
    $(div).append(html); 
    document.getElementById('analysisnum').innerHTML = data.length;
    }  
}
...
function group_analyze_confirm_button(){
    ...
    var admin = $('#useremail').text();
  	var job = {"submit_user":admin,"task_name":group_name, "uid_list":group_confirm_uids,'task_max_count':group_analysis_count};
  	//console.log(job);
  	$.ajax({
  	    type:'POST',
  	    url: group_ajax_url,
