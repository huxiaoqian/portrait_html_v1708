...
Draw_dis_Table:function(data){
	...
	for (i=0;i<data.length;i++){
		...
		html += '<tr><td title='+data[i][0]+' style="word-break:break-all;">'+data[i][0]+'</td><td style="word-break:break-all;">'+data[i][1]+'</td><td>'+data[i][2]+'</td><td>'+dis_type+'</td><td>'+data[i][4]+'</td><td><progress style="width:78px;" value="'+data[i][5]+'" max="100"></progress>&nbsp;&nbsp;'+data[i][5]+'%</td><td><a href="javascript:void(0)" id="group_commit_analyze">提交分析</a>&nbsp;&nbsp;&nbsp;<a href="javascript:void(0)" id="group_commit_control" >提交监控</a>&nbsp;&nbsp;<a href="javascript:void(0)" id="task_del">删除</a></td></tr>';
		//j += 10;
	}
	html += '</tbody>';
	html += '</table>';
...
function draw_table(data,div){
    ...
    for (var i=0;i<data.length;i++) {
        ...
        html += '<tr><td style="text-align:center">' + data[i][0] + '</td>';
        html += '<td style="text-align:center">' + uname + '</td>';
        html += '<td style="text-align:center">' + data[i][2].toFixed(2) + '</td><td style="text-align:center">' + data[i][3].toFixed(2) + '</td><td style="text-align:center">' + data[i][4].toFixed(2) + '</td><td><input name="analyze_list_option" class="search_result_option" type="checkbox" value="' + '1' + '" /></td></tr>';
    	//i = i + 1;
 	}
    html += '</table>'; 
    $(div).append(html); 
