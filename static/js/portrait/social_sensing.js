...
function so_draw_control_table(data){
	...
	var item_name = '';
	var html='';
    html += '<table class="table table-striped table-bordered bootstrap-datatable datatable responsive" style="overflow-y:auto;word-wrap: break-word;word-break: break-all;table-layout:fixed;">';
    html += '<tr><th width="50" style="text-align:center">头像</th>';
    html += '<th style="text-align:center">昵称</th>';
    html += '<th style="text-align:center">身份</th>';
    html += '<th style="text-align:center">领域</th>';
    html += '<th style="text-align:center">身份敏感度</th>';
    html += '<th style="text-align:center">影响力</th><th style="text-align:center">活跃度</th></tr>';//<th><input name="so_user_choose_all" id="so_user_choose_all" type="checkbox" value="" onclick="so_user_choose_all()" /></th>
...
function draw_sensor(data){
 ...
 //    html += '</div>';
    if (item_sensor.length == 0){
	    ...
	    html += '<table style="margin-top:10px;font-weight:lighter;word-wrap: break-word;word-break: break-all;table-layout:fixed;" class="table table-striped table-bordered bootstrap-datatable datatable responsive" >';
	    html += '<tr><th width="50" style="text-align:center">头像</th>';
        html += '<th style="text-align:center">昵称</th>';
        html += '<th style="text-align:center">身份</th>';
        html += '<th style="text-align:center">领域</th>';
        html += '<th style="text-align:center">身份敏感度</th></tr>';
        for (var i=0;i<item.length;i++) {
	        ...
	        html += '<tr><td style="text-align:center"><img class="img-circle shadow-5"  style="width:30px;height:30px;" title="'+item[i][0]+'"  src="' + item_img + '" ></td>';
